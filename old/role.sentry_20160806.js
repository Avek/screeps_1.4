...
module.exports = {
    run: function(creep) {
        ...
        var targetRoom = creep.room.name;//"W42S26";
        if (!creep.memory.area) {
            if (_.sum(Game.creeps, (c) => c.memory.role == 'sentry' && c.memory.area == "W41S27") == 0) {
                creep.memory.area = "W41S27";
            } else if (_.sum(Game.creeps, (c) => c.memory.role == 'sentry' && c.memory.area == "W41S26") == 0) {
                creep.memory.area = "W41S26";
            } else if (_.sum(Game.creeps, (c) => c.memory.role == 'sentry' && c.memory.area == "W42S26") == 0) {
                creep.memory.area = "W42S26";
            } else if (_.sum(Game.creeps, (c) => c.memory.role == 'sentry' && c.memory.area == "W42S27") == 0) {
                creep.memory.area = "W42S27";
            } else if (_.sum(Game.creeps, (c) => c.memory.role == 'sentry' && c.memory.area == "W43S26") == 0) {
